...
class BapProjectTask(models.Model):
    ...
    def send_weekly_task_report(self):
        # Lọc các dự án có trạng thái 'open'
        projects = self.env['bap.project'].search([('status', '=', 'open')])

        for project in projects:
            # Tạo một dictionary lưu thông tin task theo trạng thái và người tham gia
            task_info = {
                'new': {},
                'dev': {},
                'qc': {},
            ...
            for task in tasks:
                if task.status not in task_info:
                    continue
                if task.dev_id:
                    if task.dev_id.id not in task_info[task.status]:
                        task_info[task.status][task.dev_id.name] = 0
                    task_info[task.status][task.dev_id.name] += 1
                if task.qc_id:
                    if task.qc_id.id not in task_info[task.status]:
                        task_info[task.status][task.qc_id.name] = 0
                    task_info[task.status][task.qc_id.name] += 1

            # Gửi email cho Project Manager (pm_id) của project
            pm_user = project.pm_id
            if pm_user:
                email_body = f"Weekly Task Report for Project: {project.project_name}\n\n"
                for status, users in task_info.items():
                    email_body += f"\nStatus: {status.capitalize()}\n"
                    for user_name, task_count in users.items():
                        email_body += f"{user_name}: {task_count} tasks\n"

                # Gửi email
                template = self.env.ref('bap_project.email_template_task_report_weekly')
                template.sudo().send_mail(pm_user.id, force_send=True)
