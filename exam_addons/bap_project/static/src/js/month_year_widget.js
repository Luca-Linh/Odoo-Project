odoo.define('bap_project.month_year_format', function (require) {
    "use strict";
    ...
    // Modified DateWidget to be used for month and years
    var MonthYearDateWidget = DateWidget.extend({
        ...
        _parseMonthYearDate: function (value, field, options) {
            ...
            if (date.isValid()) {
                ...
                if (date.year() >= 1900) {
                    date.date(1); // Set day to the first day of the month
        ...
        _parseClient: function (v) {
            var date = this._parseMonthYearDate(v, null, { timezone: false });
            if (date) {
                date.date(1); // Ensure it's always the first day of the month
            }
            return date;
        }
    });
    ...
    var MonthYearFormatFieldDate = FieldDate.extend({
        ...
        _convertMonthYearDateFormat: function (dateFormat) {
            ...
            if (!isletter.test(dateFormat[0])) {
                // If we have some . or / or - at the beginning of the string, remove it.
        ...
        _formatValue: function (value) {
            ...
            var date = moment.utc(value); // Parse the date
            date.date(1); // Ensure it's always the first day of the month
