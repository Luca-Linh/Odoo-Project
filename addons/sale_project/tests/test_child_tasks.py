...
class TestNestedTaskUpdate(SavepointCase):
    ...
    def test_creating_subtask_user_id_on_parent_dont_go_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'user_id': self.user.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id, 'user_id': False})
        self.assertFalse(child.user_id)

    def test_creating_subtask_partner_id_on_parent_goes_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.user.partner_id.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id})
        self.assertEqual(child.partner_id, self.user.partner_id)

    def test_creating_subtask_email_from_on_parent_goes_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'email_from': 'a@c.be'})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id})
    ...
    def test_creating_subtask_sale_line_id_on_parent_goes_on_child_if_same_partner_in_values(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id, 'sale_line_id': self.order_line.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': self.partner.id, 'parent_id': parent.id})
    ...
    def test_creating_subtask_sale_line_id_on_parent_dont_go_on_child_if_other_partner(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id, 'sale_line_id': self.order_line.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': self.user.partner_id.id, 'parent_id': parent.id})
    ...
    def test_write_user_id_on_parent_dont_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'user_id': False})
        child = self.env['bap_project.task'].create({'name': 'child', 'user_id': False, 'parent_id': parent.id})
        self.assertFalse(child.user_id)
    ...
    def test_write_partner_id_on_parent_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': False})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': False, 'parent_id': parent.id})
    ...
    def test_write_email_from_on_parent_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent'})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id})
    ...
    def test_write_sale_line_id_on_parent_write_on_child_if_same_partner(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id, 'partner_id': self.partner.id})
        self.assertFalse(child.sale_line_id)
    ...
    def test_write_sale_line_id_on_parent_dont_write_on_child_if_other_partner(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'parent_id': parent.id, 'partner_id': self.user.partner_id.id})
        self.assertFalse(child.sale_line_id)
        parent.write({'sale_line_id': self.order_line.id})
        self.assertFalse(child.sale_line_id)
    ...
    def test_linking_user_id_on_parent_dont_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'user_id': self.user.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'user_id': False})
        self.assertFalse(child.user_id)
    ...
    def test_linking_partner_id_on_parent_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.user.partner_id.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': False})
        self.assertFalse(child.partner_id)
        child.write({'parent_id': parent.id})
        self.assertEqual(child.partner_id, self.user.partner_id)
    ...
    def test_linking_email_from_on_parent_write_on_child(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'email_from': 'a@c.be'})
        child = self.env['bap_project.task'].create({'name': 'child', 'email_from': False})
        self.assertFalse(child.email_from)
        child.write({'parent_id': parent.id})
        self.assertEqual(child.email_from, 'a@c.be')
    ...
    def test_linking_sale_line_id_on_parent_write_on_child_if_same_partner(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id, 'sale_line_id': self.order_line.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': self.partner.id})
        self.assertFalse(child.sale_line_id)
        child.write({'parent_id': parent.id})
        self.assertEqual(child.sale_line_id, parent.sale_line_id)
        parent.write({'sale_line_id': False})
    ...
    def test_linking_sale_line_id_on_parent_write_dont_child_if_other_partner(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id, 'sale_line_id': self.order_line.id})
        child = self.env['bap_project.task'].create({'name': 'child', 'partner_id': self.user.partner_id.id})
        self.assertFalse(child.sale_line_id)
        self.assertNotEqual(child.partner_id, parent.partner_id)
        child.write({'parent_id': parent.id})
        self.assertFalse(child.sale_line_id)
    ...
    def test_writing_on_parent_with_multiple_tasks(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'user_id': False, 'partner_id': self.partner.id})
        children_values = [{'name': 'child%s' % i, 'user_id': False, 'parent_id': parent.id} for i in range(5)]
        children = self.env['bap_project.task'].create(children_values)
    ...
    def test_linking_on_parent_with_multiple_tasks(self):
        parent = self.env['bap_project.task'].create({'name': 'parent', 'partner_id': self.partner.id, 'sale_line_id': self.order_line.id, 'user_id': self.user.id})
